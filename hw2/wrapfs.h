...
static inline int wrapfs_set_write_dirty(struct inode *i,int val)
{
	WRAPFS_I(i)->w_dirty = val;
	return WRAPFS_I(i)->w_dirty;
}
static inline int wrapfs_get_write_dirty(struct inode *i)
{
	return WRAPFS_I(i)->w_dirty;
}
...
/*Adding xaattr functions*/
#ifndef EXTRA_CREDIT
#define CHKSUM_SIZE 16
#endif
#define XATTR_HAS_INTEGRITY "user.has_integrity"
#define XATTR_INTEGRITY_VAL "user.integrity_val"

#define READ_DIRTY_BIT 0
#define WRITE_DIRTY_BIT 1
...
extern int calculate_checksum(struct file*,void*,int);

#ifdef EXTRA_CREDIT
#define XATTR_INTEGRITY_TYPE "user.integrity_type"
extern int get_chksum_size(char *algorithm_name);
extern int get_default_chksum_size(struct dentry *dentry,char *default_algo,int *size);
#endif
