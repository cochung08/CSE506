			 ...
			 int mode, struct nameidata *nd)
{
	...
	if(NULL == has_integrity)
	{
		err = -ENOMEM;
		goto out;
	}
	parent_dentry = dget_parent(lower_path.dentry);
	if(vfs_getxattr(parent_dentry,XATTR_HAS_INTEGRITY,has_integrity,1)>0)
	{
		if(!memcmp(has_integrity,"1",1))
		{
			if(vfs_setxattr(lower_path.dentry,XATTR_HAS_INTEGRITY,has_integrity,1,XATTR_CREATE)>0 && !S_ISDIR(lower_dentry->d_inode->i_mode))
			{
				setxattr_file = dentry_open(lower_path.dentry,lower_path.mnt,O_RDONLY,current_cred());
				if(!getchkbuf)
					if(calculate_checksum(setxattr_file,getchkbuf,CHKSUM_SIZE)>=0)
						vfs_setxattr(lower_path.dentry,XATTR_INTEGRITY_VAL,getchkbuf,CHKSUM_SIZE,XATTR_CREATE);
...
static int wrapfs_mkdir(struct inode *dir, struct dentry *dentry, int mode)
{
	...
	if(vfs_getxattr(parent_dentry,XATTR_HAS_INTEGRITY,has_integrity,1)>0)
        {
                if(!memcmp(has_integrity,"1",1))
                        vfs_setxattr(lower_path.dentry,XATTR_HAS_INTEGRITY,has_integrity,1,XATTR_CREATE);
        }
	dput(parent_dentry);
