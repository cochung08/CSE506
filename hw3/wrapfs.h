...
#ifdef DEBUG_SUPPORT
#define UDBGE(rc) printk(KERN_DEFAULT "DBG:%s:%s:%d:Error=%d\n",__FILE__,__func__,__LINE__,rc);
#endif
...
/* wrapfs super-block options with reference to ecryptfs_mount_crypt_stat*/
struct wrapfs_mount_options
{
	int mmap;
#ifdef DEBUG_SUPPORT
	int debug_flag;
#define WRAPFS_DEBUG_SUPERBLOCK_OP	 0x01
#define WRAPFS_DEBUG_INODE_OP		 0x02
#define WRAPFS_DEBUG_DENTRY_OP		 0x04
#define WRAPFS_DEBUG_FILE_OP		 0x10
#define WRAPFS_DEBUG_ADDRESS_SPACE_OP	 0x20
#define WRAPFS_DEBUG_OTHER_OP		 0x40
#endif
};
/* wrapfs super-block data in memory */
struct wrapfs_sb_info {
	struct super_block *lower_sb;
	struct wrapfs_mount_options mount_options;
#ifdef WRAPFS_CRYPTO
...
#ifdef WRAPFS_CRYPTO
extern int wrapfs_decrypt_page(struct page *dec_page, struct page *page, char *key);
extern int wrapfs_encrypt_page(struct page *enc_page, struct page *page, char *key);
extern int calculate_key(char *key,char *chkbuf,int len);
#endif
#ifdef DEBUG_SUPPORT
extern int debug_support(struct super_block *sb, char *ops);
#endif
