...
static int wrapfs_parsing_options(struct super_block *sb, int *check_uid)
{
	char *p;
	int token;
	...
	WRAPFS_SB(sb)->mount_options.mmap = 0;
#ifdef DEBUG_SUPPORT
	...
	if(options!=NULL){
		while((p = strsep(&options,","))!= NULL){
			...
			switch(token){
				case wrapfs_option_mmap:
					WRAPFS_SB(sb)->mount_options.mmap = 1;
#ifdef WRAPFS_CRYPTO
					WRAPFS_SB(sb)->key[0] = 0;
#endif
					break;
#ifdef DEBUG_SUPPORT
				case wrapfs_option_debug:
					...
					WRAPFS_SB(sb)->mount_options.debug_flag = db_flg;
					break;
#endif
				default:
					printk(KERN_WARNING
...
static int wrapfs_read_super(struct super_block *sb, void *raw_data, int silent)
{
	...
	wrapfs_parsing_options(lower_sb,0);
			    ...
			    const char *dev_name, void *raw_data)
{
	...
	{	
		len = strlen((char *)raw_data);
		options = kmalloc(sizeof(char)*len,GFP_KERNEL);
		if(options!=NULL)
			strcpy(options,raw_data);
		else
			printk("No Memory, mounted options may not work\n");
	}
